  ...
  return (
    <div className="flex justify-between flex-col p-6 w-[360px] border-l border-[var(--g-color)] bg-[var(--main-color)] h-[var(--height)]">
      <div className="flex-col flex-nowrap w-full h-min">
        <div className="w-full py-2 text-lg font-semibold mb-5 text-[var(--w-color)] flex  gap-12 items-center">
            Edit group settings
        ...
        <div className="mb-4" ref={menuRef}>
          {brothersDataArray.map((brother) => (
            <GroupSidebarCard
              ...
              isActive={activeUser === brother} // Pass if it's active
              onClick={() => setActiveUser(activeUser === brother ? null : brother)} // Toggle menu
            />
          ))}
        </div>
      </div>
