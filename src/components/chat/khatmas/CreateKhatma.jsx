...
export default function CreateKhatma() {
  ...
  const validateForm = async () => {
    ...
    if (!selectedStartDate)
      newErrors.selectedStartDate = "Date from is required";
    ...
    if (
      ...
      newErrors.selectedEndDate = "End date must be after start date.";
    if (isLimited === true && (!userLimit || userLimit <= 0))
      newErrors.userLimit = "Please enter a valid number.";
    if (isLimited === null)
      newErrors.isLimited =
        "Please choose whether the khatma is limited or not";
  ...
  return (
    <div className="no-scrollbar">
      <div
        className={`relative overflow-hidden no-scrollbar ${showConfirmation ? "w-[380px] h-[200px]" : "w-[700px] h-[600px]"}`}
      >
        <div
          className={`no-scrollbar absolute inset-0 transition-opacity duration-500 overflow-y-auto hide-scrollbar ${
            showConfirmation
              ? "opacity-0 pointer-events-none hidden w-1"
          ...
          <div className="max-h-[1680px] pb-4 bg-[var(--main-color)] pt-4 px-4 rounded-sm border border-[var(--g-color)] flex flex-col">
            ...
            <div className="flex flex-col relative pl-6 gap-6">
              ...
              <form className="flex gap-2 text-[var(--w-color)] flex-col">
                ...
                />
                {errors.khatmaName && (
                  <p className="text-[var(--r-color)]">{errors.khatmaName}</p>
                )}
              </form>
              ...
              <form className="flex gap-2 text-[var(--w-color)] flex-col">
                ...
                {errors.isLimited && (
                  <p className="text-[var(--r-color)]">{errors.isLimited}</p>
                )}
              ...
              <container className="text-[var(--w-color)] flex-col gap-2">
                ...
                {errors.fromChapter && (
                  <p className="text-[var(--r-color)]">{errors.fromChapter}</p>
                )}
                {errors.fromVerse && (
                  <p className="text-[var(--r-color)]">{errors.fromVerse}</p>
                )}
                {errors.fromPage && (
                  <p className="text-[var(--r-color)]">{errors.fromPage}</p>
                )}
                ...
                {errors.toChapter && (
                  <p className="text-[var(--r-color)]">{errors.toChapter}</p>
                )}
                {errors.toVerse && (
                  <p className="text-[var(--r-color)]">{errors.toVerse}</p>
                )}
                {errors.toPage && (
                  <p className="text-[var(--r-color)]">{errors.toPage}</p>
                )}
              </container>
              ...
              {/* Description Section */}
              <form action="">
                <h2 className="text-lg font-semibold text-[var(--w-color)] ">
                  Description :
                </h2>
                <textarea
                ...
                />
                {errors.description && (
                  <p className="text-[var(--r-color)]">{errors.description}</p>
                )}
              </form>

              {apiError && (
                <div className="text-[var(--r-color)]">{apiError}</div>
              )}
            </div>
      ...
      >
        <div className="w-[360px] h-[200px] flex justify-center items-center border border-[var(--g-color)] flex-col p-6 bg-[var(--main-color)]">
          <div className="text-[var(--w-color)] items-center justify-center text-lg text-center">
            A confirmation request has been sent to the group admin. Please wait
            for their approval.
          </div>
        </div>
      </div>
