...
export default function ProfilePopup({ user, onClose, position }) {
  ...
  useEffect(() => {
    if (position && popupRef.current) {
      ...
      const popupHeight = rect.height; // measure real height
      const padding = 16;
  ...
  }, [position, showReportForm]); // re-run when form toggles so height updates
  ...
  return (
    <div
      ref={popupRef}
      className="absolute z-50 w-96 p-6 rounded-2xl shadow-xl bg-white border"
      style={{
        top: coords.top,
        left: coords.left,
